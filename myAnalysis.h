...
const TString beamName[] = {"^{132}Sn","^{108}Sn","^{112}Sn","^{124}Sn"};
const TString targetName[] = {"^{124}Sn","^{112}Sn","^{124}Sn","^{112}Sn"};

const Int_t   nParticle  = 5;
const TString pidName[]  = {"Proton","Deuteron","Triton","Helium3","Alpha","6He","6Li","7Li"};
...
void loadAnaTree(int systemID=-1, bool isLoose=false/*loose event selection*/, bool isAll=false/*use full stat.*/)
{
	...
	for(auto i: beamID){
		...
		c[i]->SetBranchAddress("isTPCRecoEvent",&isTPCRecoEvent);
...
//		vertexCutter->GetObject(Form("fitVxy_%dSn",beamA[i]),fitVxy[i]);
	...
	//	Double_t vxyPar[6]; fitVxy[i]->GetParameters(vxyPar);
		...
		TCut cutTPCReco  = "isTPCRecoEvent";
		...
		TCut cutTotal    = cutRuns+cutMinBias+cutBeam+cutTPCReco+cutGGClose+cutVertexZ+cutVertexXY+cutVxBxCor+cutVyByCor;
		if(isLoose){
			...
			cutTotal    = cutRuns+cutMinBias+cutBeam+cutTPCReco+cutVertexZ+cutVertexXY;
		}
		if(isAll){
...
const Double_t trigB0_RMS[]   = { 0.003978, 0.003185 , 0.002129, 0.004981 };

//const Int_t    nMultBin   = 5; // same as FOPI, and no-centrality cut
const Int_t    nMultBin   = 4; 
const TString  centralTitle[] = {"b_{0}#leq0.15","0.15<b_{0}#leq0.25","0.25<#it{b}_{0}#leq0.45","trig.bias"};
